...
# @@Created          :  Friday, Sep 06, 2024 12:36 EDT
# @@File             :  entrypoint.sh
...
SERVICE_USER="blueonyx"  # execute command as another user
SERVICE_GROUP="blueonyx" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for blueonyx"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
